  ...
  import '../js/rem.js'
    import c from '../css/swiper.min.css'
  import a from '../css/reset.css'
  import b from '../css/picture.css'
  ...
  $(function() {
    ...
    const arr = []
    const img = require('../image/photo.png')
    ...
    $.ajax({
      ...
      success: function(res) {
        ...
        let imgs = list.imgSrcs.split(',')
        let images = []
        document.title = list.title
        $.each(imgs, function(index, value) {
          images.push(`<div class="swiper-slide"><img src="${value}" ></div>`)
        })

        $('.swiper-wrapper').html(images)
        $('nav img').prop('src', imgs[0])
        ...
        $('.article').html(list.description)

        var pictureAll =$(".swiper-wrapper .swiper-slide").length
        var isNoSwiping = true;
        if(pictureAll < 2){
          isNoSwiping = false
        }
    ...
    const filter = { limit: 20, where: { type: 2 } }
    $.ajax({
      url: '/api/posts',
      type: 'get',
      dataType: 'json',
